...
def cv2imread(path, mode):
    ...
    if isinstance(path,str):
        unicode_compatible_path = path
    elif isinstance(path,unicode):
        unicode_compatible_path = path.encode('utf-8')
    else:
...
def imsave(path, img):
    ...
    """
    if len(img.shape) != 3:
        raise AssertionError('The image tobe saved must have shape [height, width, num_channels]. It is now %s'
                             %(str(img.shape)))
    img = np.clip(img, 0, 255).astype(np.uint8)
    if img.shape[2] == 1:
        # If grayscale, just take out the last dimension.
        img = img[...,0]
...
def read_preprocessed_file_path_list(preprocessed_file_path_list):
    with open(preprocessed_file_path_list,'r') as f:
...
def rgb_to_yuv(image):
    if len(image.shape) == 3:
        ...
        return cv2.cvtColor(np.array(image,dtype=np.uint8),cv2.COLOR_RGB2YUV)
    elif len(image.shape) == 4:
        ret = []
        for img_i in range(image.shape[0]):
            ret.append(rgb_to_yuv(image[img_i,...]))
        return np.array(ret)
    else:
