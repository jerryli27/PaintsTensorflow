...
learning_rate_for_save=0.001 # 0.001
learning_rate_actual=0.0002 # 0.001
iterations=800000# 800000
batch_size=4  # The larger the better.
actual_batch_size=32  # The larger the better.
content_weight=5
...
checkpoint_output='output_checkpoint/colorsketches-adv_net-%s-hint-%s-iter-%d-batchsize-%d-lr-%f-content-%d_%%s.jpg' % (str(use_adversarial_net), str(use_hint), iterations, batch_size, learning_rate_for_save, content_weight)
output='output/colorsketches-adv_net-%s-hint-%s-iter-%d-batchsize-%d-lr-%f-content-%d.jpg' % (str(use_adversarial_net), str(use_hint), iterations, batch_size, learning_rate_for_save, content_weight)
model_save_dir='model/colorsketches-adv_net-%s-hint-%s-iter-batchsize-%d-%d-lr-%f-content-%d/' % (str(use_adversarial_net), str(use_hint), iterations, batch_size, learning_rate_for_save, content_weight)
...
os.system('python ~/PycharmProjects/my-neural-style/color_sketches.py --learning_rate=%f --iterations=%d --batch_size=%d --content_weight=%d --checkpoint_iterations=%d --width=%d --height=%d --checkpoint_output=%s --test_img=%s --test_img_hint=%s --output=%s --model_save_dir=%s --print_iterations=%d %s %s %s %s'
          % (learning_rate_actual, iterations, actual_batch_size, content_weight, checkpoint_iterations, width, height, checkpoint_output, test_img, test_img_hint, output, model_save_dir, print_iteration, do_restore_and_train_string, do_restore_and_generate_string, use_adversarial_net_string, use_hint_string))
