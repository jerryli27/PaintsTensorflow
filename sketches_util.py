...
def generate_training_batch(image1, image2, train = False, minimize=False, log=False, leak=(0,0), bin_r=0.9, dtype=np.float32):
    assert len(image1.shape) == 4 and len(image2.shape)==4
    num_image = image1.shape[0]

    image1_processed = []
    image2_processed = []
    for i in range(num_image):
        current_image1, current_image2 = generate_training_image_pair(image1[i,...], image2[i,...], train = train, minimize=minimize, log=log, leak=leak, bin_r=bin_r, dtype=dtype)
        image1_processed.append(current_image1)
        image2_processed.append(current_image2)
    image1_processed = np.array(image1_processed, dtype=dtype)
    image2_processed = np.array(image2_processed, dtype=dtype)
    return image1_processed, image2_processed
