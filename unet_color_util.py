...
def net(image, mirror_padding=False,reuse=False):
    ...
    assert len(image_shape) == 4 and image_shape[1] >= 32 and image_shape[2] >= 32 and \
           image_shape[1] % 16 == 0 and image_shape[2] % 16 == 0  # Otherwise the conv fails.
    prev_layer = image
    ...
    with tf.variable_scope('unet', reuse=reuse):
        ...
        for i in range(len(CONV_UP_NUM_FILTERS) - 1):
            ...
            if i % 2 == 0:
                ...
                if prev_layer_shape[1] != layer_to_be_concatenated_shape[1] or prev_layer_shape[2] != layer_to_be_concatenated_shape[2]:
                    if not (abs(prev_layer_shape[1] - layer_to_be_concatenated_shape[1]) == 0 and abs(
                                prev_layer_shape[2] - layer_to_be_concatenated_shape[2]) == 0):
        ...
        # final = conv_layer(prev_layer, num_filters=CONV_UP_NUM_FILTERS[-1],
        #            filter_size=CONV_UP_KERNEL_SIZES[-1], strides=CONV_UP_STRIDES[-1],
        #            with_bias=True,
        #            mirror_padding=mirror_padding, norm='', name='final', reuse=reuse)
        ...
        if not (image_shape[0] == bw_shape[0] and image_shape[1] == bw_shape[1] and image_shape[2] == bw_shape[2]):
            raise AssertionError('image_shape and bw_shape are different. image_shape = %s and bw_shape = %s' %(str(image_shape), str(bw_shape)))
